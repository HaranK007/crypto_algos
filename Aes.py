# AES S-box
s_box = (
    
)

# AES Rcon
r_con = (
    (0x00, 0x00, 0x00, 0x00),
    (0x01, 0x00, 0x00, 0x00),
    (0x02, 0x00, 0x00, 0x00),
    (0x04, 0x00, 0x00, 0x00),
    (0x08, 0x00, 0x00, 0x00),
    (0x10, 0x00, 0x00, 0x00),
    (0x20, 0x00, 0x00, 0x00),
    (0x40, 0x00, 0x00, 0x00),
    (0x80, 0x00, 0x00, 0x00),
    (0x1B, 0x00, 0x00, 0x00),
    (0x36, 0x00, 0x00, 0x00)
)

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]

def shift_rows(state):
    for i in range(1, 4):
        state[i] = state[i][i:] + state[i][:i]

def mix_columns(state):
    for i in range(4):
        a = state[i][0]
        b = state[i][1]
        c = state[i][2]
        d = state[i][3]

        state[i][0] = (0x02 * a) ^ (0x03 * b) ^ c ^ d
        state[i][1] = a ^ (0x02 * b) ^ (0x03 * c) ^ d
        state[i][2] = a ^ b ^ (0x02 * c) ^ (0x03 * d)
        state[i][3] = (0x03 * a) ^ b ^ c ^ (0x02 * d)

def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]

def key_expansion(key):
    round_keys = [key]

    for i in range(1, 11):
        prev_key = round_keys[i - 1]

        # Generate new key
        new_key = []
        word = [prev_key[j][3] for j in range(4)]
        word = word[1:] + word[:1]
        word = [s_box[b] for b in word]
        word[0] ^= r_con[i][0]

        for j in range(4):
            new_key.append([prev_key[k][j] ^ word[k] for k in range(4)])

        round_keys.append(new_key)

    return round_keys

def aes_encrypt(key, plaintext):
    round_keys = key_expansion(key)
    state = [[plaintext[i + j] for j in range(4)] for i in range(0, 16, 4)]

    add_round_key(state, round_keys[0])

    for rnd in range(1, 10):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, round_keys[rnd])

    sub_bytes(state)
    shift_rows(state)
    add_round_key(state, round_keys[10])

    ciphertext = bytes(sum(state, []))
    return ciphertext

def aes_decrypt(key, ciphertext):
    round_keys = key_expansion(key)
    state = [[ciphertext[i + j] for j in range(4)] for i in range(0, 16, 4)]

    add_round_key(state, round_keys[10])

    for rnd in range(9, 0, -1):
        shift_rows(state)
        sub_bytes(state)
        add_round_key(state, round_keys[rnd])
        mix_columns(state)

    shift_rows(state)
    sub_bytes(state)
    add_round_key(state, round_keys[0])

    plaintext = bytes(sum(state, []))
    return plaintext

# Example usage
key = bytes.fromhex("2b7e151628aed2a6abf7158809cf4f3c")  # 128-bit key
plaintext = bytes.fromhex("3243f6a8885a308d313198a2e0370734")  # 128-bit plaintext
ciphertext = aes_encrypt(key, plaintext)
decrypted_text = aes_decrypt(key, ciphertext)

print("Plaintext:", plaintext.hex())
print("Ciphertext:", ciphertext.hex())
print("Decrypted text:", decrypted_text.hex())
